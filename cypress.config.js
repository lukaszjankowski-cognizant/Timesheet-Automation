        ...
        onMessage: {
          async switchToTabAndGetContent (browser) {
            ...
            const page = await retry(async () => {
              ...
              const page = pages.find((page) => page.url().includes('cognizantonline')) //change to correct url
              ...
              if (!page) throw new Error(`Could not find the page with the URL containing`+{page}+`Current pages: ${pages.map(page => page.url()).join(', ')}`);
            ...
            //const paragraph = (await page.waitForSelector('#PTNUI_LAND_REC_GROUPLET_LBL\\$0'))
            const paragraph = (await page.waitForSelector('#div_qkActioncontainer > div > div:nth-child(2) > div > div.row.row1 > div:nth-child(1)')) //change for the correct selector on your page
